...
`}const b=(e=100)=>{let{scrollTop:t,scrollHeight:n,clientHeight:r}=document.documentElement;return t+r>=n-e},x=e=>{let t=!1;return()=>{t||(t=!0,requestAnimationFrame(()=>{e(),t=!1}))}},S=({onLoadMore:e,threshold:t=100,shouldLoad:n=()=>!0})=>{let r=()=>{b(t)&&n()&&e()},i=x(r);return window.addEventListener(`scroll`,i,{passive:!0}),()=>{window.removeEventListener(`scroll`,i)}};function C(){return`
  ...
  `}const w={products:[],total:0,loading:!1,categories:[],categoriesLoading:!1,searchValue:``,selectedCategory1:``,selectedCategory2:``,selectedSort:`price_asc`,selectedLimit:20,currentPage:1,isLoadingMore:!1,hasNext:!0,cartCount:0};let T=async function(e){let t=document.body.querySelector(`#root`),n=window.location.pathname,r=n.match(/(?:\/front_6th_chapter1-1)?\/product\/([^/]+)/),a;if(n===`/`||n===``||n.includes(`limit`)||n.includes(`sort`)||n.includes(`search`)||n.includes(`front_6th_chapter1`)&&r===null||n.includes(`category`))a=s({content:`
        ...
        `,cartCount:e.cartCount,showBackButton:!1,title:`쇼핑몰`});else if(r){let t=n.split(`/`)[2],r=await p(t);e.productDetail=r,a=s({content:`${y({productInfo:r})}`,cartCount:e.cartCount,showBackButton:!0,title:`상품 상세`})}else a=s({content:`${C()}`,title:`쇼핑몰`});t.innerHTML=a};async function E(){w.loading=!0,w.categoriesLoading=!0,w.cartCount=localStorage.getItem(`cartCount`),T(w);let e=window.location.pathname;if(e.includes(`sort`)){let t=e.match(/sort=([^&/]+)/);t&&(w.selectedSort=t[1])}if(e.includes(`limit`)){let t=e.match(/limit=([^&/]+)/);t&&(w.selectedLimit=t[1])}let[{products:t,pagination:{total:n}},r]=await Promise.all([f({sort:w.selectedSort,limit:w.selectedLimit}),m()]);w.products=t,w.total=n,w.loading=!1,w.categories=r,w.categoriesLoading=!1,T(w)}const D=()=>r(async()=>{let{worker:e,workerOptions:t}=await import(`./browser-YiGTYyvf.js`);return{worker:e,workerOptions:t}},[]).then(({worker:e,workerOptions:t})=>e.start(t));D().then(E);function O(){document.querySelectorAll(`[data-category1]:not([data-category2])`).forEach(e=>{e.onclick=e=>{if(w.selectedCategory1=e.target.getAttribute(`data-category1`),w.selectedCategory2=``,e.target.matches(`[data-category1]:not([data-category2])`)){let t=e.target.getAttribute(`data-category1`);v.navigateTo(`/category1=${encodeURIComponent(t)}`)}}}),document.querySelectorAll(`[data-category2]`).forEach(e=>{e.onclick=e=>{if(w.selectedCategory1=e.target.getAttribute(`data-category1`),w.selectedCategory2=e.target.getAttribute(`data-category2`),e.target.matches(`[data-category2]`)){let t=e.target.getAttribute(`data-category1`),n=e.target.getAttribute(`data-category2`);v.navigateTo(`/category1=${encodeURIComponent(t)}&category2=${encodeURIComponent(n)}`)}}});let e=document.querySelector(`[data-breadcrumb='category1']`);e&&(e.onclick=e=>{let t=e.target.getAttribute(`data-category1`);!t&&e.target.closest(`[data-category1]`)&&(t=e.target.closest(`[data-category1]`).getAttribute(`data-category1`)),t&&v.navigateTo(`/category1=${encodeURIComponent(t)}`),w.selectedCategory1=e.target.getAttribute(`data-category1`),w.selectedCategory2=``});let t=document.getElementById(`limit-select`);t&&(t.onchange=e=>{let t=e.target.value;A({limit:t,page:w.currentPage}),w.selectedLimit=Number(t),v.navigateTo(`/limit=${encodeURIComponent(w.selectedLimit)}`)});let n=document.getElementById(`sort-select`);n&&(n.onchange=e=>{let t=e.target.value;w.selectedSort=t,A({sort:w.selectedSort,page:w.currentPage}),v.navigateTo(`/sort=${encodeURIComponent(w.selectedSort)}`)});let r=document.getElementById(`search-input`);r&&(r.addEventListener(`keydown`,e=>{let t=e.target.value.trim();t!==``&&e.key===`Enter`&&(A({search:t,page:w.currentPage}),w.searchValue=t,v.navigateTo(`/search=${encodeURIComponent(t)}`))}),document.body.addEventListener(`click`,e=>{if(!e._productLinkHandled){let t=e.target.closest(`[data-product-link]`);if(t){e.preventDefault(),e._productLinkHandled=!0;let n=t.getAttribute(`data-product-link`);v.navigateTo(`/product/${n}`)}}}));let i=document.getElementById(`product-list`)||document.body;i._cartListenerAttached!==!0&&(i.addEventListener(`click`,e=>{let t=e.target.closest(`.add-to-cart-btn`);if(t){e.preventDefault();let t=Number(localStorage.getItem(`cartCount`))||0,n=t+1;localStorage.setItem(`cartCount`,n),T(w)}}),i._cartListenerAttached=!0)}const k=T;T=function(...e){let t=k.apply(this,e);return O(),t};const A=async({limit:e=20,sort:t=`price_asc`,search:n=``,page:r=1})=>{w.loading=!0,w.isLoadingMore=!0;try{let{products:i,pagination:{total:a,page:o,hasNext:s}}=await f({limit:e,sort:t,search:n,page:r});w.currentPage===o?(w.products=i,w.loading=!1):w.products=[...w.products,...i],w.isLoadingMore=!1,w.total=a,w.currentPage=o,w.hasNext=s,w.selectedSort=t,w.selectedLimit=e,T(w)}catch(e){w.loading=!1,console.log(e)}},j=()=>S({onLoadMore:()=>{A({limit:w.selectedLimit,sort:w.selectedSort,search:w.searchValue,page:Number(w.currentPage)+1})},threshold:100,shouldLoad:()=>!w.isLoadingMore&&w.hasNext});j();